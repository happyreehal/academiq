...
export default function OtpVerification({ 
  email, otp, setOtp, loading, resendTimer,
  ...
  return (
    <form onSubmit={onSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div style={{ textAlign: "center" }}>
        <button type="button" onClick={onResend} disabled={resendTimer > 0 || loading} className="btn-resend">
          {resendTimer > 0 ? `🔄 Resend OTP in ${resendTimer}s` : "🔄 Resend OTP"}
        </button>
      </div>

      {/* Back Button */}
      <button type="button" onClick={onBack} className="btn-back-step">
        ← Change Details
      </button>
    </form>
  );
