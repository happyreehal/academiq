    ...
    <div className="card">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
        ...
        <span style={{ 
          ...
          color: "#1D9E75", borderRadius: "2px", padding: "6px 14px", 
      ...
      {filteredPapers.length === 0 ? (
        ...
        filteredPapers.map((paper) => (
          <div key={paper.public_id} className="paper-card">
            <div>
              <div style={{ fontWeight: "500", color: "white", fontSize: "15px", marginBottom: "6px" }}>
                {paper.subject ?? "Unknown"} — <span style={{ color: "#1D9E75" }}>{paper.academic_year ?? "N/A"}</span>
              </div>
              <div style={{ color: "rgba(255,255,255,0.5)", fontSize: "12px", marginBottom: "4px", letterSpacing: "0.5px" }}>
                {paper.department ?? "—"} | {paper.class_name ?? "—"} | {paper.semester ?? "—"} Semester
              </div>
              <div style={{ color: "rgba(255,255,255,0.3)", fontSize: "11px" }}>
                By: {paper.uploaded_by ?? "Unknown"}
              </div>
            </div>

            <div style={{ display: "flex", gap: "12px" }}>
              <a 
                href={viewUrl(paper.file_url)} target="_blank" rel="noreferrer"
                style={{
                  padding: "8px 16px", background: "rgba(29,158,117,0.1)", color: "#1D9E75", 
                  borderRadius: "4px", fontSize: "12px", fontWeight: "500", 
                  textDecoration: "none", border: "1px solid rgba(29,158,117,0.3)", transition: "all 0.3s"
                }}
              >
                👁️ View
              </a>
              <a 
                href={downloadUrl(paper.file_url)} target="_blank" rel="noreferrer"
                style={{
                  padding: "8px 16px", background: "linear-gradient(135deg, #1D9E75, #0d7a5a)", 
                  color: "white", borderRadius: "4px", fontSize: "12px", fontWeight: "500", 
