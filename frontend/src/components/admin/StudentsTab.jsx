export default function StudentsTab({ students, approveStudent, rejectStudent, removeStudent }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
        ...
        {pendingStudents.length === 0 ? (
          ...
          pendingStudents.map(s => (
            <StudentItem 
              ...
              actions={
                <>
                  <button className="btn-approve" onClick={() => approveStudent(s.email)}>
                    ✅ Approve
                  </button>
                  <button className="btn-reject" onClick={() => rejectStudent(s.email)}>
                    ❌ Reject
                  </button>
                </>
              }
        ...
        {activeStudents.length === 0 ? (
          ...
          activeStudents.map(s => (
            <StudentItem 
              ...
              student={s}
              actions={
                <button className="btn-danger" onClick={() => removeStudent(s.email)}>
                  🗑️ Remove
                </button>
              }
            />
      ...
      {/* REJECTED STUDENTS - Only show if any exist */}
